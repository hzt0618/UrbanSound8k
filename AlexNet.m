...
for i = 1:length(uniqueLabels)
    ...
    for j = 1:length(trainFiles)
        ...
        img = imresize(img, [227, 227]);  % 调整图像大小为227x227
...
% 获取输入层的大小
inputSize = net.Layers(1).InputSize;  % AlexNet输入大小是[227 227 3]
disp(['输入层的大小: ', num2str(inputSize(1:2))]);
...
% 修改AlexNet的最后一层
layers = net.Layers;
layers(end-2) = fullyConnectedLayer(5, 'WeightLearnRateFactor',10, 'BiasLearnRateFactor',10);  % 修改最后的全连接层为5类输出
layers(end) = classificationLayer;  % 替换为分类层
...
options = trainingOptions('sgdm', ...
    ...
    'Verbose', false, ...
    'OutputFcn', @(info) saveProgress(info, trainLoss, trainAccuracy));  % 输出损失和准确度
...
function saveProgress(info, trainLoss, trainAccuracy)
    ...
    if info.State == "done"
        ...
        print(gcf, 'AlexNet.png', '-dpng', '-r300');  % 使用300 DPI的分辨率保存
    end
end